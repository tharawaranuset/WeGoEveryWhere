import { pgTable, index, foreignKey, serial, numeric, varchar, date, time, integer, text, doublePrecision, check, jsonb, timestamp, primaryKey } from "drizzle-orm/pg-core"
...
export const users = pgTable("users", {
	...
	check("users_age_check", sql`age > 20`),
]);
...
export const drizzleMigrations = pgTable("_drizzle_migrations", {
	...
	name: text().notNull(),
	runOn: timestamp("run_on", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
});
